...
# dir
DATA_DIR = r'C:\Users\harol\Desktop\Thesis\Coding\RankingEmotion\extra train2000'
BIN_DIR = 'D:/ESD/training_data/extra train2000'

# Strength score for english subset (0011-0020) of ESD data.
list_file = r'C:\\Users\\harol\\Desktop\\Thesis\\Comparing Datasets\\snet_2000_new\\snet_2000.csv'
...
def get_melspectrograms(sound_file, fs=FS, fft_size=FFT_SIZE):
    ...
    mel_basis = librosa.filters.mel(
        fs, fft_size, n_mels)  # (n_mels, 1+n_fft//2)
...
def data_generator(file_list, bin_root, frame=False, batch_size=1):
    ...
    while True:
        ...
        filename = [file_list[index+x].split(',')[0]
        ...
        strength = [float(file_list[x+index].split(',')[1])
                    for x in range(batch_size)]
        strength = np.asarray(strength).reshape([batch_size])
        frame_strength = np.array(
            [strength[i]*np.ones([feat.shape[1], 1]) for i in range(batch_size)])
        # add Multi-task
        emo_class = [emo_label.index(
            str(file_list[x+index].split(',')[0].split('/')[1])) for x in range(batch_size)]
        emo_target = utils.to_categorical(
            emo_class, num_classes=4)  # one-hot encoding
        index += batch_size
